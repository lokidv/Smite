...
const CONFIG_DIR = process.env.OVPN_CONFIG_DIR || '/etc/ovpn';
const CONFIG_PATH = path.join(CONFIG_DIR, 'ovpn.json');
...
function loadConfig() {
    ...
    if (!fs.existsSync(CONFIG_PATH)) {
        ...
        console.log('[ovpn] Config created at', CONFIG_PATH);
        console.log('[ovpn] Save these credentials securely:');
        console.log('[ovpn] API Key:', generated.apiKey);
        console.log('[ovpn] Admin password:', initialPassword);
        console.log('[ovpn] Admin path:', generated.adminPath);
        const port = process.env.OVPN_PORT || '4000';
        console.log('[ovpn] Admin panel: http://<server-ip>:' + port + '/' + generated.adminPath + '/');
        return generated;
    }
