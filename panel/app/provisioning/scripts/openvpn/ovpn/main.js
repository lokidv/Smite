...
const logger = require('logger').createLogger('ovpn.log');
...
const ovpn = require('./lib/ovpn-service');
const { resolveCreateVolume, resolveUpdateVolume } = require('./lib/volume-params');

const httpPort = Number(process.env.OVPN_PORT || 4000);
const ENFORCE_INTERVAL_MS = Number(process.env.OVPN_ENFORCE_INTERVAL_MS || 30_000);
...
async function addVpn(req, res, query) {
    ...
    if (!name) {
        return sendJson(res, { success: false, error: 'publicKey (name) is required' }, 400);
    }

    const existingConf = ovpn.readClientConf(name);
    if (existingConf) {
        return sendText(res, existingConf);
    }

    const result = ovpn.addClient(name, {
...
async function handlePublicApi(req, res, pathname, query) {
    ...
    switch (pathname) {
        ...
        case 'update': {
            const result = ovpn.updateClient(getClientName(query), {
        ...
        case 'info': {
            const result = ovpn.infoClient(getClientName(query));
            return sendJson(res, result, result.success === false ? 404 : 200);
        }
        case 'disable': {
            const result = ovpn.disableClient(getClientName(query), query.reason || 'manual');
            return sendJson(res, result, result.success === false ? 400 : 200);
        }
        case 'enable': {
            const result = ovpn.enableClient(getClientName(query));
            return sendJson(res, result, result.success === false ? 400 : 200);
        }
        case 'remove': {
            const result = ovpn.removeClient(getClientName(query));
            return sendJson(res, result, result.success === false ? 400 : 200);
        }
        case 'list': {
            const result = ovpn.listClients(query);
            return sendJson(res, result, result.success === false ? 500 : 200);
        }
        case 'check': {
            const name = getClientName(query);
            return sendText(res, ovpn.clientExists(name) ? 'true' : 'false');
        }
        default:
...
async function handleAdminApi(req, res, pathname, query = {}) {
    ...
    if (pathname === adminApiPath('enforce') && req.method === 'POST') {
        ...
        const result = ovpn.enforceClients();
        return sendJson(res, result, result.success === false ? 500 : 200);
    }
    ...
    if (pathname === adminApiPath('settings') && req.method === 'GET') {
        ...
        return sendJson(res, ovpn.getServerSettings(), 200);
    }
    ...
    if (pathname === adminApiPath('settings') && req.method === 'PATCH') {
        ...
        const body = await readJsonBody(req);
        const result = ovpn.updateServerSettings(body);
        return sendJson(res, result, result.success === false ? 400 : 200);
    }
    ...
    if (route) {
        ...
        if (route.action === 'conf/download' && req.method === 'GET') {
            const conf = ovpn.readClientConf(route.name);
            if (!conf) {
                return sendJson(res, { success: false, error: 'Config not found' }, 404);
            }
            const filename = `${route.name}.ovpn`;
        ...
        if (route.action === 'qrcode' && req.method === 'GET') {
            const result = ovpn.generateClientQrPng(route.name);
            if (!result.success) {
        ...
        if (route.action === 'refresh-conf' && req.method === 'POST') {
            const result = ovpn.refreshClientConf(route.name);
            return sendJson(res, result, result.success === false ? 400 : 200);
        }

        if (route.action === 'disable' && req.method === 'POST') {
            return sendJson(res, ovpn.disableClient(route.name, 'manual'), 200);
        }

        if (route.action === 'enable' && req.method === 'POST') {
            return sendJson(res, ovpn.enableClient(route.name), 200);
        }

        if (!route.action && req.method === 'PATCH') {
            const body = await readJsonBody(req);
            const result = ovpn.updateClient(route.name, {
        ...
        if (!route.action && req.method === 'DELETE') {
            return sendJson(res, ovpn.removeClient(route.name), 200);
        }
    }
...
function serveAdminPanel(req, res) {
    ...
    const inject = `<script>window.OVPN_ADMIN_BASE='/${ADMIN_PATH}';</script>`;
...
function runEnforce() {
    const result = ovpn.runScript(['enforce']);
    if (result.stdout) {
        try {
