...
const translations: Record<Language, Translations> = {
  en: {
    ...
    installNode: {
      ...
      cleanTakeoverDesc: 'Safely resets Smite node connection and dead tunnels. Existing WireGuard (/etc/wireguard), OpenVPN (/etc/openvpn), and 3x-ui user configs, keys, and clients are strictly preserved and never deleted.',
      cleanTakeoverResult: 'Clean Takeover & Migration',
      cleanTakeoverResultDesc: 'Old Smite panel services and dead tunnel configs were cleaned. All user VPNs (WireGuard, OpenVPN, 3x-ui) were preserved intact.',
      nodeName: 'Node Name',
    ...
    tunnels: {
      ...
      zapretPresetMci: '📱 Hamrah-e Avval (MCI) - Multisplit + TTL Fooling',
      zapretPresetMtn: '📱 Irancell (MTN) - Fakedsplit + Badsum/Badseq',
      ...
      zapretSplitPosHint: 'Packet offset to split TLS ClientHello / UDP packet. Use 2, 1, 3, or sniext.',
  ...
  fa: {
    ...
    installNode: {
      ...
      cleanTakeoverDesc: 'فقط اتصال نود و تونل‌های مرده‌ی Smite را ریست می‌کند. کلیه کانفیگ‌ها و کاربران وایرگارد (/etc/wireguard)، اوپن‌وی‌پی‌ان (/etc/openvpn) و 3x-ui کاملاً دست‌نخورده باقی می‌مانند و به هیچ‌وجه پاک نخواهند شد.',
      cleanTakeoverResult: 'پاکسازی و مهاجرت از پنل قبلی',
      cleanTakeoverResultDesc: 'سرویس‌ها و تونل‌های پنل قبلی پاکسازی شدند. کلیه سرویس‌ها و کاربران وایرگارد و OpenVPN با امنیت کامل حفظ شدند.',
      nodeName: 'نام نود',
    ...
    tunnels: {
      ...
      zapretPresetMci: '📱 همراه اول (MCI) - Multisplit + فریب TTL',
      zapretPresetMtn: '📱 ایرانسل (MTN) - Fakedsplit + Badsum/Badseq',
      ...
      zapretSplitPosHint: 'محل شکستن پکت، مثل عدد ۲، ۱ یا ۳.',
