...
## ۵) کدام Desync Mode را انتخاب کنم؟
...
## ۷) مرجع: معادل دستی (فقط برای درک)
...
Create it from **Tunnels → Create Tunnel → Core: Zapret**, pick the node, and tune the desync strategy (start with `fake` / `badseq,ts` / fake SNI `hcaptcha.com`). Smite installs per-tunnel NFQUEUE `iptables` chains (never a global flush) and runs/stops `nfqws` for you. The companion proxy (`xray` + `config.json`) is configured separately on the same host, as shown above. Requires `NET_ADMIN` + `NET_RAW`, which are pre-configured in both the Docker and native installs.
