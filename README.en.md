...
### Step 1 — Get the offline bundle

**Option A (recommended) — download a pre-built bundle from [GitHub Releases](https://github.com/lokidv/Smite/releases/latest):**

Bundles are built automatically by GitHub Actions (see `.github/workflows/offline-bundle.yml`) for every release tag. Download the one that matches your server's **architecture and OS** on any machine that has internet (your laptop, an abroad VPS, ...):

| Server OS | Python | File |
| --- | --- | --- |
...
> The Python wheels inside the bundle must match the server's Python version — pick the file for your OS. Check with `python3 -V` on the server. Regardless of the file name, the folder inside extracts as `smite-offline-<arch>`.

**Option B — build it yourself (on a machine WITH internet):**
...
Build or download a fresh bundle on the internet-connected machine, transfer it, extract it over the previous directory, and re-run the same installer — data (`/opt/smite/panel/data`, certs, `.env`) is preserved.
...
- **[Offline Native Install (No Docker)](#-offline-native-install-no-docker)** — download a pre-built bundle from Releases (or build one) and install the panel/nodes on restricted/Iran servers with `systemd` + Python `venv`.
